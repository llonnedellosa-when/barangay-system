...
<body>
    ...
    <div class="report-header">
        <div style="font-size:8pt; color:#555;">Republic of the Philippines &bull; Province of [Province] &bull;
            Municipality of [Municipality]</div>
        <div class="brgy-name">Barangay [Name]</div>
        <div class="report-title">{{ $doc->doc_type_label }}</div>
        <div class="report-sub">Year {{ $year }} @if ($doc->purok_filter)
                &bull; {{ $doc->purok_filter }}
            @endif
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            @forelse($residents as $i => $r)
                <tr>
                    <td>{{ $i + 1 }}</td>
                    <td style="font-family:monospace; font-size:7.5pt;">{{ str_pad($r['id'], 4, '0', STR_PAD_LEFT) }}
                    </td>
                    ...
                    @if ($doc->doc_type === 'masterlist' || $doc->doc_type === 'new_residents')
                        <td>
                            ...
                            @endif
                            @if ($r['is_senior_citizen'])
                                <span class="tag tag-s">SC</span>
                            @endif
                            @if ($r['is_pwd'])
                                <span class="tag tag-p">PWD</span>
                            @endif
                            @if ($r['is_4ps'])
                                <span class="tag tag-f">4Ps</span>
                            @endif
                        </td>
                    @endif
                    @if ($doc->doc_type === 'youth_report')
            ...
            @empty
                <tr>
                    <td colspan="12" style="text-align:center; padding:20px; color:#999;">No records found.</td>
                </tr>
            @endforelse
        </tbody>
        <tfoot>
    ...
    <div class="footer">
        Barangay [Name] &bull; {{ $doc->doc_type_label }} &bull; Doc No. {{ $doc->doc_number }} &bull; Generated:
        {{ $date }}
    </div>
</body>
